...
function yz_get_user_tools( $user_id = null, $icons = null ) {
	...
	// Get Ajax Nonce
	$ajax_nonce = wp_create_nonce( 'yz-tools-nonce-' . $user_id );

	?>

	<div class="yz-tools yz-user-tools yz-tools-<?php echo $icons; ?>" data-nonce="<?php echo $ajax_nonce; ?>" data-user-id="<?php echo $user_id; ?>" data-component="profile">
...
function yz_add_user_link_verification_icon( $html, $user_id ) {
	...
	// Return;
	return '<a href="' . $url . '">' . $display_name . $verified_icon . '</a>' ;
}
...
add_filter( 'bp_activity_allowed_tags', 'yz_allowed_activity_actions_tags' );
