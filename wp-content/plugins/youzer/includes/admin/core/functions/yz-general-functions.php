...
function yz_move_wp_fields_to_bp_notice() {
    ...
    if ( ! yz_option( 'yz_social_login_upgrade_1' ) ) { ?>

        <div class="notice notice-warning">

            <a href="<?php echo add_query_arg( 'yz-dismiss-extension-notice', 'yz_social_login_upgrade_1', yz_get_current_page_url() ); ?>" type="button" class="yz-delete-notice"></a>

            <p><strong>Youzer - Social Login Notice:<br></strong></p>
            <p>Recently instagram changed their API again and the old callback url format is not valid anymore, so we made some changes on the code.</p>
            <p>if you are using social login buttons or instagram widget consider renewing to this new callback urls :</p>
            <ul>
                <li><strong>Facebook</strong> : <?php echo home_url( '/yz-auth/social-login/Facebook' ); ?></li>
                <li><strong>Google</strong> : <?php echo home_url( '/yz-auth/social-login/Google' ); ?></li>
                <li><strong>Twitter</strong> : <?php echo home_url( '/yz-auth/social-login/Twitter' ); ?></li>
                <li><strong>Instagram</strong> : <?php echo home_url( '/yz-auth/social-login/Instagram' ); ?></li>
                <li><strong>Linked In</strong> : <?php echo home_url( '/yz-auth/social-login/LinkedIn' ); ?></li>
                <li><strong>Twitch</strong> : <?php echo home_url( '/yz-auth/social-login/TwitchTV' ); ?></li>
                <li><strong>Instagram Widget</strong> : <?php echo home_url( '/yz-auth/feed/Instagram' ); ?></li>
            </ul>
            ...
            <p><strong>Ps:</strong> All these urls are case sensitive so please copy them as they are written !</p>
...
function yz_display_new_extension_notice() {
    ...
    if ( isset( $_GET['yz-dismiss-extension-notice'] ) ) {
        update_option( $_GET['yz-dismiss-extension-notice'], 1 );   
    }
