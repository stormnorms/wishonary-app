...
class ACADP_Admin_Payments {
	...
	public function admin_menu() {	
		add_submenu_page(
			...
			__( 'Advanced Classifieds and Directory Pro - Payment History', 'advanced-classifieds-and-directory-pro' ),
			__( 'Payment History', 'advanced-classifieds-and-directory-pro' ),
			'manage_acadp_options',
			'edit.php?post_type=acadp_payments'
		);	
	}

	/**
	 * Move "Payment History" submenu under our plugin's main menu.
	 *
	 * @since  1.7.3
	...
	public function parent_file( $parent_file ) {	
		...
		if ( 'acadp_payments' == $current_screen->post_type ) {
			$submenu_file = 'edit.php?post_type=acadp_payments';
		...
		if ( 'acadp_payments' == $typenow ) {		
			...
			foreach ( $post_ids as $post_id ) {				
				if ( ! $this->update_payment_status( $action, $post_id ) ) {
					wp_die( __( 'Error updating post.', 'advanced-classifieds-and-directory-pro' ) );
				}
		...
		if ( 'edit.php' == $pagenow && 'acadp_payments' == $post_type ) {		
			...
			foreach ( $allowed_actions as $action ) {
				...
				if ( isset( $_REQUEST[ $action ] ) && ! empty( $_REQUEST[ $action ] ) ) {
					$message = sprintf( _n( "Payment set to $_action.", "%s payments set to $_action.", (int) $_REQUEST[ $action ], 'advanced-classifieds-and-directory-pro' ), number_format_i18n( $_REQUEST[ $action ] ) );
					break;
				}
			}
