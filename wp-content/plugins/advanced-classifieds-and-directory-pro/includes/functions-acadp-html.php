...
function the_acadp_address( $post_meta, $term_id ) {
	...
	if ( ! empty( $post_meta['phone'][0] ) ) {
		$show_phone_number_publicly = ! empty( $general_settings['show_phone_number_publicly'] ) ? 1 : 0;

		echo '<span class="acadp-phone">';
		echo '<span class="glyphicon glyphicon-earphone"></span>&nbsp;';
		if ( $show_phone_number_publicly ) {
	...
	if ( ! empty( $post_meta['email'][0] ) ) {		
		$show_email_address_publicly = ! empty( $general_settings['show_email_address_publicly'] ) ? 1 : 0;
...
function the_acadp_pagination( $numpages = '', $pagerange = '', $paged = '' ) {	
     ...
     */
  	...
  	/** 
   	 ...
   	 * function. 
   	 */
	$arr_params = array( 'order', 'sort', 'view', 'lang', 'renew' );
